    ...
    <section className="section-padding bg-white relative overflow-hidden">
      ...
      <div className="container-custom relative z-10">
        <div className="max-w-4xl mb-20">
          ...
          <h2 className="text-5xl md:text-6xl lg:text-7xl mb-10 leading-[1.05] text-gray-900">
            Em que Nível está
            <br />
          ...
          <p className="text-xl md:text-2xl text-gray-600 leading-relaxed">
            ...
            <span className="text-gray-900 font-semibold">A maioria está no nível 1 ou 2</span>.
          </p>
        </div>
          ...
          {levels.map((item, index) => (
            ...
            >
              <div className={`w-10 h-10 rounded-lg flex items-center justify-center mb-4 ${
                ...
                <item.icon className={`w-5 h-5 ${activeLevel === index ? 'text-white' : 'text-gray-900'}`} strokeWidth={2} />
              </div>
              
              <div className={`text-3xl font-bold mb-2 tracking-tight ${activeLevel === index ? 'text-white' : 'text-gray-900'}`}>
                {item.level}
              </div>
              
              <div className={`text-sm font-semibold mb-1 ${activeLevel === index ? 'text-gray-50' : 'text-gray-900'}`}>
                {item.title}
              </div>
          ...
          <div className="bg-gray-50 border border-gray-200 rounded-2xl p-10">
            <div className="flex items-center gap-4 mb-8">
              ...
              <div>
                ...
                <h3 className="text-2xl font-bold text-gray-900">{levels[activeLevel].title}</h3>
              </div>
            </div>
          ...
          <div className="space-y-6">
            ...
            {levels.map((item, index) => (
              ...
              >
                ...
                <div className="flex-1">
                  <div className={`text-sm font-semibold ${index === activeLevel ? 'text-white' : 'text-gray-900'}`}>{item.title}</div>
                  <div className={`text-xs ${index === activeLevel ? 'text-white/70' : 'text-gray-500'}`}>{item.subtitle}</div>
                </div>

                {index === activeLevel && (
                  <div className="text-xs px-3 py-1 bg-white text-gray-900 rounded-full font-semibold">
                    Você está aqui
                  </div>
                )}
            ...
            <div className="mt-8 p-6 bg-gray-50 border border-gray-200 rounded-xl">
              <p className="text-sm text-gray-600 leading-relaxed">
                <span className="text-gray-900 font-semibold">A mentoria executiva</span> foi desenhada para levar clínicas do nível 1-2 para o nível 4 em 9 semanas.
              </p>
            </div>
          </div>
        </div>
